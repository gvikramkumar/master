  ...
  getInitialData() {
    ...
    switch (this.report.type) {
      ...
      case 'dept-upload':
        ...
        break;
      case 'input-data':
        prmMeasure = this.pgLookupService.callRepoMethod('getSubmeasureForSystemInputData','',{params:this.moduleId}).toPromise();
        break;
    }
  ...
  handleHasMultiSmAndFiscalMonth() {
    ...
    switch (this.report.type) {
      ...
      case 'mapping-upload':
        ...
        break;
      case 'input-data':
        obs = this.pgLookupService.callRepoMethod('getInputDataFiscalMonthsFromSubmeasureKeys', { submeasureKeys: this.submeasureKeys },{params:this.moduleId});
        break;
    }
    obs.subscribe(fiscalMonths => {
      this.fiscalMonths = fiscalMonths.map(fm => Number(fm)).sort().reverse().slice(0, 24)
