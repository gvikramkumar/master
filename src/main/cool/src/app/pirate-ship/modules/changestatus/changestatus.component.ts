...
export class ChangestatusComponent implements OnInit {
  ...
  basicmodule_hint = {
    ...
    "Offer_Attribution": "<ol><li>BUPM must get the proposed OA split approved by Global Revenue and the OA Board.</li><li> The BUPM, BUC, and TSPM must be engaged in this conversation.</li><li>The BUPM must create the OA child PID(s) in eGenie (PID type is SW Subscription Mapped SKU) to dictate where the revenue will be routed.</li><li>Create an OA child PID for each group that is receiving revenue (i.e. a PID for the BU and a PID for SOA).</li><li> It is possible to leverage existing OA child PIDs if the revenue is being routed to the same team</li><li>The BU Controller must approve these PIDs before moving on to the next step<li>5. Once the OA child PIDs are approved in eGenie, BUPM or Offer Lead must complete the OA Mapping Template (PDT can download latest version of template in eGenie</li><li>BUPM must submit completed template to PDT by opening an NPI Workspace Case.</li><li>PDT will complete the OA setup and will reach out to Global Revenue to approve</li><li> Once Global Revenue has approved, the Offer Lead can click the \"Mark as Complete\" button.</li></ol>",
