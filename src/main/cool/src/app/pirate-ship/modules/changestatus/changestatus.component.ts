...
import { PirateShipSharedService } from '../../../services/pirate-ship-shared.service';
...
export class ChangestatusComponent implements OnInit {
  ...
  basicmodule_hint = {
    ...
    "Test_Orderability": "1. NPPM must create a test order in CCW to ensure all aspects of the ordering process are functioning properly and there are no gaps or issues.\n 2. BUPM and Offer Lead should meet with the NPPM to complete the ordering process in CCW and confirm all ordering rules are correct.\n3. After the order is created, Offer Lead should track the order to ensure the order closes as expected and flows to SBP properly without any system issues.",
    "Offer_Attribution": "1. BUPM must get the proposed OA split approved by Global Revenue and the OA Board. The BUPM, BUC, and TSPM must be engaged in this conversation.\n2. The BUPM must create the OA child PID(s) in eGenie (PID type is SW Subscription Mapped SKU) to dictate where the revenue will be routed\n3. Create an OA child PID for each group that is receiving revenue (i.e. a PID for the BU and a PID for SOA). It is possible to leverage existing OA child PIDs if the revenue is being routed to the same team\n4. The BU Controller must approve these PIDs before moving on to the next step\n5. Once the OA child PIDs are approved in eGenie, BUPM or Offer Lead must complete the OA Mapping Template (PDT can download latest version of template in eGenie\n6. BUPM must submit completed template to PDT by opening an NPI Workspace Case.\n7.PDT will complete the OA setup and will reach out to Global Revenue to approve\n8. Once Global Revenue has approved, the Offer Lead can click the \"Mark as Complete\" button.",
    "NPI_Licensing": "SW Licensing Stakeholder ensures that NPI licensing activities are complete ",
    "Royalty_Setup": "1. BUPM must engage with the NPPM to fill out the Royalty Configuration Template. BUPM can provide either a reference PID from a past offer that has the same royalties or provide attributes related to the supplier and product.\n2. The NPPM will work with the Royalty team to complete the royalty setup. Once complete, the BUPM can validate the PID royalty setup using the Royalty Rate Master Report (RRM).",
    "Export Compliance": "1. BUPMs must complete the Export Product Review Form: <a href=\"https://pepd.cloudapps.cisco.com/legal/export/pepd/EPReviewForm.do?method=showEPRForm#!/\" target=\"_blank\">EPR Form</a> \n2. Once the BUPM has completed the form, they will receive an email notification with the results, as well as an EPR number.\n3. Offer Leads or BUPMs should document the case number as it may be required for creation of new PIDs.\n4. Offer Leads or BUPMs should reach out to the GET Team to schedule a due diligence review. The following three points are important to note before the due diligence review:\n    a. The BUPM will need to explain the correlation between PID and the product the customer receives.\n   b. Any SW associated with the SBP offering needs an export compliance due diligence review\n   c. If the SW requires a formal US government review, the formal government review may potentially take up to 2-3 months\n5. Once the compliance due diligence review is complete, an email will be sent from the GET Team (exportclass@cisco.com) stating the PIDs are compliant.\n6. If emails need to be sent to the GET Team (exportclass@cisco.com) requesting confirmation that export compliance due diligence is complete, please be sure to reference the EPR # in the subject line of the email."
  };
  ...
  markAsCompleteModal(labelName: string) {
    ...
    this.infohelp = 'Are you sure you would like to mark this as complete? Once completing a module, you will not be able to move it back to \"In-progress\"';
    this.ishide = !this.ishide;
  }
